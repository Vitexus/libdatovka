...
int main(int argc, char **argv) {
    ...
    TEST("Empty structure", test_isds_message_status_change_free, &message_status_change);

    TEST_CALLOC(message_status_change);
    TEST_FILL_STRING(message_status_change->dmID);
    TEST_CALLOC(message_status_change->time);
    TEST("Full structure", test_isds_message_status_change_free, &message_status_change);
