...
int main(void) {
    ...
    /* Get list of received unread messages */
    {
        ...
        printf("Getting list of unread received messages\n");
        err = isds_get_list_of_received_messages(ctx, &from_time, NULL, NULL,
                MESSAGESTATE_ANY & ~MESSAGESTATE_READ, 0, &number, &messages);
        if (err)
    ...
    if (last_message_id) {
        ...
        printf("Marking as read last received message with ID: %s\n",
                last_message_id);
        err = isds_mark_message_read(ctx, last_message_id);
        if (err)
            printf("isds_mark_message_read() failed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_mark_message_read() succeeded:\n");
    ...
    /* Mark as read message with invalid ID*/
    {
        struct isds_message *message = NULL;
        char *id = "123456789112345678921";

        printf("Marking message as read with invalid ID: %s\n", id);
