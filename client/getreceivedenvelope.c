...
int main(void) {
    ...
    if (last_message_id != NULL) {
        ...
        err = isds_get_received_envelope(ctx, last_message_id, &message);
        if (err)
            printf("isds_get_received_envelope() failed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_get_received_envelope() succeeded:\n");
    ...
    /* Download envelope with invalid ID*/
    {
        struct isds_message *message = NULL;
        char *id = "123456789112345678921";

        printf("Getting received envelope with invalid ID: %s\n", id);
        err = isds_get_received_envelope(ctx, id, &message);
        if (err)
            printf("isds_get_received_envelope() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_get_received_envelope() succeeded. "
    ...
    /* Download non-existent envelope */
    {
        struct isds_message *message = NULL;
        char *id = "7777777";

        printf("Getting non-existent received envelope with ID: %s\n", id);
        err = isds_get_received_envelope(ctx, id, &message);
        if (err)
            printf("isds_get_received_envelope() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
