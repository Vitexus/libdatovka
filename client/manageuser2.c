...
static _Bool isds_AddressExt2_eq(const struct isds_AddressExt2 *a1,
    ...
    return (a1 == a2) ||
        (str_eq(a1->adCode, a2->adCode) && str_eq(a1->adCity, a2->adCity) &&
...
int main(void) {
    ...
    /* Alter existing user description. */
    {
        ...
        /* Add address is missing. */
        {
            ...
            if (!address->adCity) {
                address->adCity = join_str("Oberwerwolfsbruck", NULL);
            }
            if (!address->adStreet) {
                address->adStreet = join_str("Pappenheimergasse", NULL);
            }
            if (!address->adNumberInStreet) {
                address->adNumberInStreet = join_str("6", NULL);
            }
            if (!address->adNumberInMunicipality) {
                address->adNumberInMunicipality = join_str("1224", NULL);
            }
            if (!address->adZipCode) {
    ...
    if (user_to_be_created) {
        ...
        err = isds_AddDataBoxUser2(ctx, db_owner_info->dbID, user_to_be_created,
                NULL, NULL, &refnumber);
        if (err) {
            printf("isds_AddDataBoxUser2() failed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
            exit(EXIT_FAILURE);
        } else {
    ...
    if (user_actually_created) {
        ...
        if (!user_actually_created->personName->pnGivenNames) {
            fprintf(stderr, "Not enough memory\n");
            exit(EXIT_FAILURE);
        }
        ...
        *user_to_be_created->userPrivils = PRIVIL_VIEW_INFO;
        ...
        err = isds_UpdateDataBoxUser2(ctx, db_owner_info->dbID,
                ...
                &refnumber);
        if (err) {
            printf("isds_UpdateDataBoxUser2() failed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
            exit(EXIT_FAILURE);
        } else {
            printf("isds_UpdateDataBoxUser2() succeeded with reference "
                "number: %s\n", refnumber);
        }
    ...
    if (user_actually_created) {
        ...
        if (err) {
            ...
            printf("isds_DeleteDataBoxUser2() succeeded with reference "
                "number: %s\n", refnumber);
        }
