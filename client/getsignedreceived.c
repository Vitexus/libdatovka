...
int main(void) {
    ...
    /* Download signed message with invalid ID*/
    {
        ...
        printf("Getting signed received message with invalid ID: %s\n", id);
        err = isds_get_signed_received_message(ctx, id, &message);
        if (err)
            printf("isds_get_signed_received_message() failed as assumed: "
                    "%s: %s\n", isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_get_signed_received_message() succeeded. "
    ...
    /* Download non-existent signed message */
    {
        ...
        printf("Getting non-existent signed received message with ID: %s\n", id);
        err = isds_get_signed_received_message(ctx, id, &message);
        if (err)
            printf("isds_get_signed_received_message() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_get_signed_received_message() succeeded. "
                    "This should not happen:\n");
