...
int main(void) {
    ...
    /* Download last message delivery info */
    if (last_message_id) {
        struct isds_message *message = NULL;

        printf("Getting last received message delivery info with ID: %s\n",
            last_message_id);
        err = isds_get_delivery_info(ctx, last_message_id, &message);
        if (err)
            printf("isds_get_delivery_info() failed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_get_delivery_info() succeeded:\n");
            print_message(message);
            save_data("Saving delivery info",
    ...
    /* Download delivery info with invalid ID */
    {
        struct isds_message *message = NULL;
        char *id = "123456789112345678921";

        printf("Getting delivery info with invalid ID: %s\n", id);
        err = isds_get_delivery_info(ctx, id, &message);
        if (err)
            printf("isds_get_delivery_info() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_get_delivery_info() succeeded. "
    ...
    /* Download non-existent delivery info */
    {
        struct isds_message *message = NULL;
        char *id = "7777777";

        printf("Getting non-existent delivery info with ID: %s\n", id);
        err = isds_get_delivery_info(ctx, id, &message);
        if (err)
            printf("isds_get_delivery_info() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
