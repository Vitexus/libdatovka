...
int main(void) {
    ...
    /* Get list of delivered unread messages */
    {
    ...
    /* Mark last message as received */
    if (last_message_id) {
        struct isds_message *message = NULL;

        printf("Marking as received last received message with ID: %s\n",
                last_message_id);
        err = isds_mark_message_received(ctx, last_message_id);
        if (err)
            printf("isds_mark_message_received() failed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_mark_message_received() succeeded:\n");
    ...
    /* Mark as received message with invalid ID*/
    {
        struct isds_message *message = NULL;
        char *id = "123456789112345678921";

        printf("Marking message as received with invalid ID: %s\n", id);
        err = isds_mark_message_received(ctx, id);
        if (err)
            printf("isds_mark_message_received() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
        else {
            printf("isds_mark_message_received() succeeded. "
    ...
    /* Mark non-existent message as received */
    {
        struct isds_message *message = NULL;
        char *id = "7777777";

        printf("Marking as received non-existent message with ID: %s\n", id);
        err = isds_mark_message_received(ctx, id);
        if (err)
            printf("isds_mark_message_received() failed as assumed: %s: %s\n",
                    isds_strerror(err), isds_long_message(ctx));
